...
import opdSample      from '../assets/OPD Sample.pdf';
import surgerySample  from '../assets/Surgery Sample.pdf';
import progressSample from '../assets/Progress Sample.pdf';
import imagingSample  from '../assets/Imaging Sample.pdf';

const CONFIG = {
  opd:      { color: '#7c3aed', label: 'OPD Note',       pdf: opdSample },
  surgery:  { color: '#ea580c', label: 'Surgery Note',    pdf: surgerySample },
  progress: { color: '#16a34a', label: 'Progress Note',   pdf: progressSample },
  imaging:  { color: '#db2777', label: 'Imaging Report',  pdf: imagingSample },
};

export default function SampleViewer() {
  const { type } = useParams();
  const safeType = type in CONFIG ? type : 'opd';
      ...
      {showPDF && (
        ...
        >
          ...
          >
            ...
            {/* iframe — most reliable cross-browser PDF embed */}
            <iframe
              src={pdf}
              title={label}
              style={{
